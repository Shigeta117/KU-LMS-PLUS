...
export const dynamic = 'force-dynamic'; // リクエスト時に env vars を読む

function buildScript(): string {
  const supabaseUrl  = process.env.NEXT_PUBLIC_SUPABASE_URL  ?? '';
  const supabaseAnon = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ?? '';

  // 設定値のみ動的注入し、残りのスクリプト本体は変更なし
  return `/* KU-LMS+ ブックマークレット本体 (generated by /api/bookmarklet) */
(function () {
  'use strict';

  if (window.__KULMS_BM_RUNNING) { console.info('[KU-LMS+] \\u65e2\\u306b\\u5b9f\\u884c\\u4e2d\\u3067\\u3059'); return; }
  ...
  function showToast(type, msg) {
    if (!_toastEl) {
      ...
      _toastEl.style.cssText = [
        ...
        'z-index:2147483647','max-width:90vw','min-width:260px','padding:12px 18px',
        'border-radius:14px','font-family:-apple-system,BlinkMacSystemFont,\\"Segoe UI\\",sans-serif',
        'font-size:14px','font-weight:600','line-height:1.4',
        'box-shadow:0 6px 24px rgba(0,0,0,.3)','transition:opacity .25s','pointer-events:none',
      ].join(';');
    ...
    if (type === 'success' || type === 'error' || type === 'warn') {
      _toastTimer = setTimeout(function () { if (_toastEl) _toastEl.style.opacity = '0'; }, 5000);
    }
  }
  ...
  async function getValidSession() {
    ...
    if (session && Date.now() < session.expiresAt - 5 * 60 * 1000) return session;
  ...
  function parseCoursePage(html, baseUrl, courseId, courseName) {
    ...
    doc.querySelectorAll('section.list-group-item.cl-contentsList_listGroupItem').forEach(function (section) {
      ...
      results.push({
        course_id: courseId, course_name: courseName || null,
        title: title, category: category,
        start_time: dateRange.start_time, deadline: dateRange.deadline,
        detail_url: detailUrl, is_submitted_lms: isSubmitted,
      });
    });
    return results;
  }
  ...
  async function scrape() {
    ...
    var courseLinks = Array.from(scheduleTable.querySelectorAll('a[href*="/webclass/course.php/"]'))
      .map(function (a) { return { url: a.href, courseName: extractCourseName(a.textContent || '') }; });
    ...
    for (var i = 0; i < courseLinks.length; i++) {
      ...
      } catch (e) { console.warn('[KU-LMS+] fetch 失敗:', item.url, e.message); }
  ...
  async function upsertAssignments(assignments, session) {
    ...
    var rows = assignments.map(function (a) {
      return {
        user_id: session.userId, course_id: a.course_id, course_name: a.course_name,
        title: a.title, category: a.category, start_time: a.start_time || null,
        deadline: a.deadline, detail_url: a.detail_url,
        is_submitted_lms: a.is_submitted_lms, updated_at: now,
      };
    });
...
export async function GET() {
  return new NextResponse(buildScript(), {
    headers: {
